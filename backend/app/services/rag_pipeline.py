...
# ✅ Load Gemini LLM or fallback to mock in test env
...
"""
    ...
    def process_query(self, query: str, db: Session) -> Dict[str, Any]:
        ...
        if intent == QueryIntent.UNKNOWN:
            llm_response = self.llm.invoke([HumanMessage(content=query)]).content
            print(f"[DEBUG] Gemini Response (LLM): {llm_response}")
            return {
                "success": True,
    ...
    def _generate_response(self, intent: QueryIntent, data: Dict[str, Any]) -> str:
        ...
        if not template or not data:
            return "I couldn't find the information. Please try rephrasing your question."
